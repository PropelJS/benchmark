...
Runner.prototype.calculateSuite = function * calculateSuite(suite) {
  ...
  var start = suite.timing._marks[0].ns;
  var stop = suite.timing._marks[marksLength - 1].ns;
...
Runner.prototype.calculateBench = function * calculateBench(bench) {
  ...
  var start = bench.timer._marks[0].ns;
  var stop = bench.timer._marks[marksLength -1].ns;
...
Runner.prototype.calculateRuns = function * calculateRuns(runs, data) {
  ...
  while (i < length) {
    var timer = runs[i];
    if(!timer.error) {
      var start = timer._marks[0].ns;
      var stop = timer._marks[timer._marks.length - 1].ns;
